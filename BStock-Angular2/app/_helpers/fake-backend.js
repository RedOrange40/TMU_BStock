            ...
            setTimeout(function () {
                ...
                if (connection.request.url.endsWith('/api/authenticate') && connection.request.method === http_1.RequestMethod.Post) {
                    ...
                    var filteredUsers = users.filter(function (user) {
                        return user.account === params_1.account && user.password === params_1.password;
                    });
                ...
                if (connection.request.url.endsWith('/api/users') && connection.request.method === http_1.RequestMethod.Post) {
                    ...
                    // validation
                    var duplicateUser = users.filter(function (user) { return user.account === newUser_1.account; }).length;
                    if (duplicateUser) {
                        return connection.mockError(new Error('帳號 "' + newUser_1.account + '"已存在 '));
                    }
                    // save new user
